# smoltrace/main.py
import os
from .utils import get_hf_user_info, generate_dataset_names, load_prompt_config, push_results_to_hf, compute_leaderboard_row, update_leaderboard
...
def run_evaluation_flow(args):
    ...
    if not hf_token:
        print("Error: HuggingFace token not found. Please provide it via --hf-token or the HF_TOKEN environment variable.")
        return
    ...
    results_repo, leaderboard_repo = generate_dataset_names(user_info['username'])
    ...
    # Push results
    push_results_to_hf(all_results, results_repo, args.model, hf_token, args.private)

    # Update leaderboard
    leaderboard_row = compute_leaderboard_row(
        args.model, all_results, trace_data, metric_data,
        dataset_used, results_repo, args.agent_type
    )
